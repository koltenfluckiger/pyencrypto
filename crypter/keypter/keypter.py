...
from ..exceptions import EncryptoMissingKeyError, EncryptoAlreadyEncryptedError, EncryptoAlreadyDecryptedError
...
class Keypter:
    ...
    def __init__(self, keyer: Union[None, Keyer] = None, private_key: Union[str, bytes, PrivateKeyTypes, None] = None, padding=OAEP(
    ...
    def load_key_from_file(self, path: Union[str, PathType], password: Union[bytes, None] = None):
        """Short summary.
    ...
    def set_key_session(self, key: Union[str, bytes, PrivateKeyTypes, None] = None) -> None:
    ...
    def already_encrypted(self, bytes):
        try:
            self.sign(bytes)
            return False
        except Exception as err:
    ...
    def already_decrypted(self, bytes):
        try:
            self.remove_sign(bytes)
            return False
        except Exception as err:
            return True
    ...
    def remove_sign(self, signed_bytes: bytes):
        ...
        if signed_bytes[0:len(self.signer)] == self.signer:
            ...
            raise EncryptoAlreadyDecryptedError(
                "Object already decrypted.")
    ...
    def encrypt_bytes(self, bytes: bytes) -> bytes:
        ...
        """
        try:
            encrypted_bytes = self.public_key.encrypt(bytes, self.padding)
    ...
    def decrypt_bytes(self, bytes: bytes) -> bytes:
        ...
        """
        try:
            decrypted_bytes = self.private_key.decrypt(bytes, self.padding)
